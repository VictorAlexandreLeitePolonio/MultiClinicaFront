...
export function MarketplaceClinicMap({ latitude, longitude, displayName, address }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [failed, setFailed] = useState(false);
  const styleUrl = process.env.NEXT_PUBLIC_MAP_STYLE_URL;

  useEffect(() => {
    if (latitude === null || longitude === null || !styleUrl || !containerRef.current) return;
    ...
    void import("maplibre-gl")
      .then((maplibregl) => {
        ...
        new maplibregl.Marker()
          ...
          .setPopup(new maplibregl.Popup({ offset: 18 }).setText(address || displayName))
          .addTo(createdMap);
