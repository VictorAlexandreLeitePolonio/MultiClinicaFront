...
import { Building2, CreditCard, Eye, History, LockOpen, Settings, Users } from "lucide-react";
...
export default function SuperAdminClinicsPage() {
  ...
  const columns: Column<SuperAdminClinic>[] = [
    ...
    { key: "document", label: "Documento", render: (clinic) => clinic.document ?? "-" },
    {
      key: "status",
      label: "Status",
      render: (clinic) => (
    ...
    { key: "usersCount", label: "Usuários", render: (clinic) => clinic.usersCount },
    { key: "createdAt", label: "Cadastro", render: (clinic) => formatDate(clinic.createdAt) },
    {
      key: "actions",
      label: "",
        ...
        <ActionsDropdown
          actions={[
            ...
            },
            {
              ...
              onClick: () => router.push(`/superadmin/clinicas/${clinic.id}?action=billing`),
              icon: <CreditCard size={14} />,
            },
            {
              label: "Registrar pagamento",
              onClick: () => router.push(`/superadmin/clinicas/${clinic.id}?action=payment`),
              icon: <CreditCard size={14} />,
            },
            {
              label: "Gerenciar usuários",
              onClick: () => router.push(`/superadmin/clinicas/${clinic.id}?tab=users`),
              icon: <Users size={14} />,
            },
            {
              label: "Histórico",
              onClick: () => router.push(`/superadmin/clinicas/${clinic.id}?tab=history`),
              icon: <History size={14} />,
            },
            {
              label: "Desbloquear cobrança",
              onClick: () => router.push(`/superadmin/clinicas/${clinic.id}?action=unblock`),
              icon: <LockOpen size={14} />,
              disabled: clinic.billingStatus !== "Blocked",
            },
          ]}
        />
      ),
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6 p-8">
      ...
      <SearchInput value={search} onChange={setSearch} placeholder="Buscar clínica..." />
