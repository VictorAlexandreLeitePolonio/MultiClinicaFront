...
export function ClinicThemePreview({
  ...
  const primary = isValidHexColor(primaryColor) ? primaryColor : DEFAULT_TENANT_THEME.primaryColor;
  const secondary = isValidHexColor(secondaryColor) ? secondaryColor : DEFAULT_TENANT_THEME.secondaryColor;
  const accent = isValidHexColor(accentColor) ? accentColor : DEFAULT_TENANT_THEME.accentColor;
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#d7f3ea] bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center gap-3 p-4 text-white" style={{ backgroundColor: secondary }}>
        ...
        <span className="truncate font-semibold">{displayName || "Nome da clínica"}</span>
      </div>
      <div className="space-y-3 p-4">
        <div className="h-2 rounded-full" style={{ backgroundColor: primary }} />
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm font-semibold text-[#0f172a] dark:text-white">Exemplo de destaque</span>
