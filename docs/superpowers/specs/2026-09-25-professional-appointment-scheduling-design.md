...
2. A data e a hora informadas no formulário representam a hora local do fuso configurado da clínica, independentemente do fuso do navegador. O servidor converte para UTC ao persistir. Datas locais inexistentes na mudança de horário de verão são recusadas com mensagem compreensível.
...
- O cadastro e a edição aceitam a data e hora locais da clínica em um campo explícito, preservando a aceitação do formato UTC atual para consumidores existentes. As respostas continuam a informar o instante em UTC. A API mantém `CreatedByUserId` como autor e `UserId` como profissional.
